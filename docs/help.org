...
It has long been recognized that the QT interval on the electrocardiogram is dependent on heart rate.  In 1920 Bazett[fn:1] was one of the first to describe this dependency in mathematical form, relating the QT interval to the square root of the RR interval.  The familiar Bazett formula, developed using primitive methodology and based on only 39 subjects, somehow has become the most commonly used formula for correcting the QT interval for heart rate---this despite the availability of a large collection of newer formulas developed from studies with better methodologies and subjects numbering up to the thousands.  How frustating for these more meticulous authors!  Further discussion of this somewhat sorry state of affairs can be found in the blog post: "Hacking the QTc."[fn:2]
...
** Reference
The reference for the formula is given, as well as the DOI link if available.  If a DOI link is present, it is possible to tap on it and go the the reference online (though be forewarned, many are behind paywalls).
** Notes
...
- *Sort order* Sort results by date, formula name or number of subjects studied.  Options for sorting by date and name also include "big 4 first" versions.  The "big 4" formulas are the ones most frequently used: QTcBZT, QTcFRD, QTcHDG, and QTcFRM (Bazett, Fridericia, Hodges, and Framingham).  These big 4 options only apply to sorting QTc formulas.
